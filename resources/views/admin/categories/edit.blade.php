                    ...
                    <form enctype="multipart/form-data" action="{{ route('categories.update', $category->id) }}" method="POST">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="InputCatTitle"  name="title" placeholder="Имя категории" value="{{ old('title',$category->title) }}">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="InputNameField"  name="name" placeholder="URL категории(latin)" value="{{ old('name',$category->name) }}">
                            </div>
                                ...
                                <select class="custom-select form-control-border" name="parent" id="selectParentCategory">
                                    ...
                                    @if(isset($categories))
                                        @foreach($categories as $categoryItem)
                                            <option value="{{ $categoryItem->id }}" @if(old('parent',$categoryItem->parent) == $categoryItem->id) SELECTED @endif>{{ $categoryItem->title }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="InputMetaTitleField"  name="meta_title" placeholder="Заголовок метатегов" value="{{ old('meta_title',$category->meta_title) }}">
                            </div>
                            <div class="form-group">
                                <label for="InputMetaDescriptionField">Описание для метатегов</label>
                                <textarea class="form-control" name="meta_description" id="InputMetaDescriptionField" rows="5">{{ old('meta_description',$category->meta_description) }}</textarea>
                            </div>
                            <div class="form-group">
                                ...
                                @else
                                <img width="80" src="{{ asset("storage/uploads/images/".$category->image)  }}" class="image"/>
                                @endif
                            </div>
